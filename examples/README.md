...
Each example pins to the most recently released version of Pulse (currently **0.12.0**, on Maven Central as `io.github.kcsurapaneni:pulse-starter`). When a new Pulse release ships, the examples are bumped in the same PR cycle so a fresh clone always works against a real published artefact.
